...
这个项目提供 vault 目录结构、智能体工作契约、提示词配置和笔记模板，让你的材料可以在不依赖云端知识库、不维护自制 Python 工具链的前提下被整理、检索和复用。

## 为什么需要 wikiR

- 本地优先：原始材料和笔记默认留在你的 vault 中。
- 智能体维护：用户用自然语言提出需求，本地智能体在后台读取、检索和写作。
- 兼容 Obsidian：笔记是普通 Markdown，使用扁平 YAML properties 和 wikilink。
- 重视证据链：原始材料先变成可追溯源卡，再被复用。
- 不绑定运行时：Hermes 或其他本地智能体可以使用自己的文件读取、OCR、搜索和文档工具。
- 人工可维护：系统主体是文件夹、提示词、模板和约定。
...
用户不应该需要手动运行维护命令。本地智能体读取 `AGENTS.md`，遵守 `90_System/` 中的提示词和模板，然后把整理后的笔记或输出写回 vault。
...
- “检查一下这个 vault 有没有断链或无依据表述。”

## 文档

- [架构说明](architecture.md)
- [Agent 操作协议](agent-runtime.md)
- [公开模板与私有 Vault 工作流](open-source-template-and-private-vault.md)
- [开发者说明](developer.md)

## 默认安全边界

默认 `.gitignore` 会忽略原始材料、附件、日志、密钥和 Obsidian 本地工作区状态。这能避免公开模板误提交私人内容。
